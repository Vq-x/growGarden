...
--[NEW UPDATE NOTES]
-- workspace.Farm:GetChildren()[6].Important.Objects_Physical IS WHERE ALL OF THE EGGS ARE STORED, OBVIOUSLY THE FARM OWNED BY LOCAL PLAYER
--
--
--
--
...
function growGarden.new()
	...
	function self:autoFavorite()
		...
		for _, plant in pairs(plantsInInventory) do
			if
				...
				or plant:GetAttribute("Variant")
					and table.find(_G.autoFavoriteVariance, plant:GetAttribute("Variant"))
			then
	...
	function self:buyPetEgg(num)
		local args = {
			num, -- one through three
		}
		game:GetService("ReplicatedStorage")
			:WaitForChild("GameEvents")
	...
	function self:autoBuyPetEgg()
		local buyableEggs = filter(workspace.NPCS["Pet Stand"].EggLocations:GetChildren(), function(v)
			return v:IsA("Model") and v:GetAttribute("RobuxEggOnly") == nil
		end)
		print(#buyableEggs)
		if #buyableEggs > 0 then
			for i = 1, 3 do
				self:buyPetEgg(i)
				task.wait(0.1)
			end
	...
	function self:autoPlaceEgg()
		local backpack = self.player.Backpack or self.player:WaitForChild("Backpack")
		local farm = self:getLocalPlayerFarm()
		local petEgg = filter(backpack:GetChildren(), function(v)
			return v:IsA("Tool") and v:GetAttribute("ITEM_TYPE") == "PetEgg"
		end)
		local totalEggsOnFarm = filter(farm.Important.Objects_Physical:GetChildren(), function(v)
			return v:IsA("Model") and v:GetAttribute("OBJECT_TYPE") == "PetEgg"
		end)
		if #totalEggsOnFarm < 3 then
			for _, egg in pairs(petEgg) do
				for i = 1, egg:GetAttribute("Uses") do
                    self.player.Character.Humanoid:EquipTool(egg)
                    task.wait(0.1)
                    self:plantEggOnFarm()
                    self:teleportToRandomPlantable()
                    task.wait(0.1)
	...
	function self:placeEgg(CF)
		local args = {
			"CreateEgg",
			-- vector.create(19.001678466796875, 0.1355254054069519, 80.30943298339844) POSITION ON FARM
			CF,
		}
		game:GetService("ReplicatedStorage")
			:WaitForChild("GameEvents")
	...
	function self:equipPet(equip, UUID, position)
		local equipType = "EquipPet"
		if equip == false then
			equipType = "UnequipPet"
		end

		local args = {
			equipType,
			-- "{938faaaf-33d5-45d4-9465-2002075545c4}",
			UUID,
			-- CFrame.new(11.850914001464844, 0, 55.820579528808594, 1, 0, 0, 0, 1, 0, 0, 0, 1) CHAR POS??
			position,
		}
		game:GetService("ReplicatedStorage")
			:WaitForChild("GameEvents")
	...
	function self:autoHatchPet()
		local farm = self:getLocalPlayerFarm()
		local petEgg = filter(farm.Important.Objects_Physical:GetChildren(), function(v)
			return v:IsA("Model") and v:GetAttribute("OBJECT_TYPE") == "PetEgg" and v:GetAttribute("TimeToHatch") <= 0
		end)
		for _, egg in pairs(petEgg) do
			self:hatchPet(egg)
		end
	...
	function self:plantEggOnFarm()
		...
		self:placeEgg(Vector3.new(randomPlantablePosition.X, 0.1355254054069519, randomPlantablePosition.Z))
	end
	...
	function self:autoCollectDroppedSeed()
		for _, v in pairs(workspace:GetChildren()) do
			if
				v:IsA("Model")
				and v:GetAttribute("OWNER") == self.player.Name
				and string.find(v.Name, "Collectable")
			then
				while v ~= nil do
	...
	function self:getLocalPlayerFarm()
	...
	function self:autoCollectPlants()
		...
		for _, proximityPrompt in pairs(proximityPrompts) do
			while _G.selling do
				self.player.Character.HumanoidRootPart.CFrame =
					CFrame.new(workspace.NPCS["Sell Stands"].PrimaryPart.Position)
				task.wait(0.1)
			end
	...
	function self:getInStockSeeds()
		local inStockSeeds = {}
		for _, v in
			pairs(game:GetService("Players").LocalPlayer.PlayerGui.Seed_Shop.Frame.ScrollingFrame:GetDescendants())
		do
			if v:IsA("Frame") then
	...
	function self:teleportToRandomPlantable()
		local farm = self:getLocalPlayerFarm()
		local plantable = filter(farm.Important.Plant_Locations:GetChildren(), function(v)
			return v.Name == "Can_Plant"
		end)
		local randomPlantable = plantable[math.random(1, #plantable)]
		self.player.Character.HumanoidRootPart.CFrame = CFrame.new(randomPlantable.Position) + Vector3.new(0, 5, 0)
	end
...
if game.GameId == Games["Grow Garden"] then
	...
	local autoFarmPlantsTab = Window:CreateTab("Auto Farm Plants")

	local autoFarmPetsTab = Window:CreateTab("Auto Farm Pets")
	...
	local autoCollectPlantsToggle = autoFarmPlantsTab:CreateToggle({
	...
	local autoCollectPlantsInterval = autoFarmPlantsTab:CreateSlider({
	...
	local autoBuySeedsToggle = autoFarmPlantsTab:CreateToggle({
		Name = "Auto Buy Seeds",
		CurrentValue = false,
	...
	local autoBuySeedsList = autoFarmPlantsTab:CreateDropdown({
	...
	local autoPlantSeedsToggle = autoFarmPlantsTab:CreateToggle({
	...
	local autoPlantSeedsList = autoFarmPlantsTab:CreateDropdown({
		Name = "Auto Plant Seeds",
		Options = garden:listPlantNames(),
		CurrentOption = nil,
		MultipleOptions = true,
	...
	local autoHatchPetToggle = autoFarmPetsTab:CreateToggle({
		Name = "Auto Hatch Pet",
		CurrentValue = false,
		Flag = "autoHatchPetToggle",
		Callback = function(Value)
			if Value then
				_G.autoHatchPetTask = task.spawn(function()
					while true do
						garden:autoHatchPet()
						task.wait(_G.autoHatchPetInterval)
					end
				end)
			else
				if _G.autoHatchPetTask then
					task.cancel(_G.autoHatchPetTask)
					_G.autoHatchPetTask = nil
				end
			end
		end,
	})

	local autoHatchPetInterval = autoFarmPetsTab:CreateSlider({
		Name = "Auto Hatch Pet Interval",
		Range = { 1, 60 },
		Increment = 1,
		Suffix = "Seconds",
		CurrentValue = 5,
		Flag = "autoHatchPetInterval",
		Callback = function(Value)
			_G.autoHatchPetInterval = Value
		end,
	})

	local autoPlaceEggToggle = autoFarmPetsTab:CreateToggle({
		Name = "Auto Place Egg",
		CurrentValue = false,
		Flag = "autoPlaceEggToggle",
		Callback = function(Value)
			if Value then
				_G.autoPlaceEggTask = task.spawn(function()
					while true do
						garden:autoPlaceEgg()
						task.wait(0.1)
					end
				...
				if _G.autoPlaceEggTask then
					task.cancel(_G.autoPlaceEggTask)
					_G.autoPlaceEggTask = nil
				end
			end
		end,
	})

	local autoBuyPetEggToggle = autoFarmPetsTab:CreateToggle({
		Name = "Auto Buy Pet Egg",
		CurrentValue = false,
		Flag = "autoBuyPetEggToggle",
		Callback = function(Value)
			if Value then
				_G.autoBuyPetEggTask = task.spawn(function()
					while true do
						garden:autoBuyPetEgg()
						task.wait(0.1)
					end
				end)
			else
				if _G.autoBuyPetEggTask then
					task.cancel(_G.autoBuyPetEggTask)
					_G.autoBuyPetEggTask = nil
				end
			end
		end,
	})

	local autoSellPlantsToggle = autoFarmPlantsTab:CreateToggle({
	...
	local autoSellPlantsAmount = autoFarmPlantsTab:CreateSlider({
	...
	local autoCollectDroppedSeedToggle = autoFarmPlantsTab:CreateToggle({
	...
	local autoFeedJimToggle = autoFarmPlantsTab:CreateToggle({
		Name = "Auto Feed Jim",
		CurrentValue = false,
	...
	local autoOpenSeedPackToggle = autoFarmPlantsTab:CreateToggle({
